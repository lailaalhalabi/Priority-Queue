/** @author Laila Alhalabi 
  ...
  * Array-based implementation of MyPQ (unsorted) 
  * @param <K> * @param <V> */
public class MyPQUnsortedArray<K, V> extends PriorityQueue<K, V> {
	...
	public MyPQUnsortedArray(KeysComparator<K> comp) { 
		super(comp); 
	} 
	...
	/** @param index * Moving the index down and modifying the min-heap till reaching the end of the heap. */
	public void downheap(int index) {
		while (hasLeft(index)) { // While the end isn't reached
			int left = getLeft(index);
			int childIndex = left;
			if (hasRight(index)) { // swapping if the right index is smaller
				int right = getRight(index);
				if (compare(elements.getElementAt(left).getKey(), elements.getElementAt(right).getKey()) > 0)
					childIndex = right; 
			}
			if (compare(elements.getElementAt(childIndex).getKey(), elements.getElementAt(index).getKey()) >= 0) 
				break;
	...
	/** Adding an entry at the end of a list.* @param key * @param value * @return Returns the created entry. */
	@Override
	...
	public Entry<K, V> min() {
		if (isEmpty())
			...
			Entry<K, V> min = elements.getElementAt(0);
		    Entry<K, V> temp = elements.getElementAt(0);
		    for (int i = 0; i < elements.size(); i++) {
		    	Entry<K, V> current = elements.getElementAt(i);
		        if (min == null || compare(current.getKey(), min.getKey()) < 1 ) {
		            min = current;
		        }
			}
			return min;
	...
	public Entry<K, V> removeMin() {
		if (isEmpty()) 
			return null;
		Entry<K,V> min = min();
		elements.removeItem(min); // Removing the min item
		return min;
	}
